            ...
            <el-table
                ...
                <el-table-column prop="operation" label="操作" width="200">
                    <template slot-scope="scope">
                        <el-button type="danger" size="mini" @click="handlePush(scope.row.id)" v-show="!(!status && showUnpush)">推送</el-button>
                        <el-button type="primary" size="mini" @click="handleUnPush(scope.row.id)" v-show="!(!status || !showUnpush)">不推送</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
    ...
    export default {
        ...
        methods: {
            handlePush (id) {
                // alert('推送')
                // alert(id)
                this.$http.post('/api/user/updateStatusList', { id, status_id: 1 }).then(res => {
                    console.log('updateStatusList',res.data)
                })
            },
            handleUnPush (id) {
                // alert('不推送')
        ...
        created () {
            this.$http.get('/api/user/getStatusList').then(res => {
                // console.log("getStatusList",res.data)
